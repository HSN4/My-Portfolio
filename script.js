...
cursorScale.forEach((link) => {
  ...
  link.addEventListener("mousemove", () => {
    ...
    if (link.classList.contains("small")) {
      cursor.classList.remove("grow"), cursor.classList.add("grow-small");
    }
  });
  link.addEventListener("mousemove", () => {
    cursor.classList.add("grow");
    if (link.classList.contains("full")) {
      cursor.classList.remove("grow"), cursor.classList.add("remove");
    }
  });
});
...
window.onscroll = function () {
  if (
    window.scrollY >= skills.offsetTop - 300 &&
    window.scrollY <= skills.offsetTop + 350
  ) {
  ...
  if (
    window.scrollY >= myWork.offsetTop - 400 &&
    window.scrollY <= myWork.offsetTop + 600
  ) {
